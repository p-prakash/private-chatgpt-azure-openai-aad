'''Add Document Page of the Streamlit App.'''
...
def upload_text_and_embeddings():
    '''Upload text and embeddings to datastore.'''
    file_name = f'{uuid.uuid4()}.txt'
    source_url = llm_helper.blob_client.upload_file(st.session_state['doc_text'],
                                                    file_name=file_name,
...
def remote_convert_files_and_add_embeddings(process_all=False):
    'Convert files and add embeddings to datastore.'
    ...
    try:
        ...
        if response.status_code == 200:
            st.success(f'{response.text}\nPlease note this is an asynchronous \
                            process and may take a few minutes to complete.')
        else:
...
def upload_file(bytes_data: bytes, file_name: str):
    ...
    if content_type == 'text/plain':
        charset = f"; charset={chardet.detect(bytes_data)['encoding']}"
    else:
        charset = ''
    st.session_state['file_url'] = llm_helper.blob_client.upload_file(
                                        bytes_data,
...
if os.getenv('AAD_Admin_SG') in st.session_state["user_groups"]:
    try:
        ...
        with st.expander("Add a single document to the knowledge base", expanded=True):
            ...
            uploaded_file = st.file_uploader("Upload a document to add it to the knowledge base",
                                             type=['pdf','jpeg','jpg','png', 'txt'])
                ...
                if st.session_state.get('filename', '') != uploaded_file.name:
                    ...
                    llm_helper.blob_client.upsert_blob_metadata(
                        uploaded_file.name,
                        {
                            ...
                            'converted_filename': parse.quote(converted_filename)
                        }
                    )
        ...
        with st.expander('Add text to the knowledge base', expanded=False):
            ...
            with col1:
                st.session_state['doc_text'] = st.text_area(
                    "Add a new text content and them click on 'Compute Embeddings'",
                    height=600)

            with col2:
                st.session_state['embeddings_model'] = st.selectbox(
                                                        'Embeddings models',
                                                        [llm_helper.get_embeddings_model()['doc']],
                                                        disabled=True)
        ...
        with st.expander('Add documents in Batch', expanded=False):
            uploaded_files = st.file_uploader(
                                'Upload a document to add it to the Azure Storage Account',
                                type=['pdf','jpeg','jpg','png', 'txt'],
                ...
                for up in uploaded_files:
                    ...
                    if st.session_state.get('filename', '') != up.name:
                        ...
                        if up.name.endswith('.txt'):
                            # Add the text to the embeddings
                            llm_helper.blob_client.upsert_blob_metadata(up.name,
                                                                        {'converted': "true"})

            col1, col2, col3 = st.columns([2,2,2])
            with col1:
                st.button("Convert new files and add embeddings",
                          on_click=remote_convert_files_and_add_embeddings)
            with col3:
                st.button("Convert all files and add embeddings",
        ...
        with st.expander("Add URLs to the knowledge base", expanded=True):
            ...
            with col2:
                st.selectbox('Embeddings models',
                             [llm_helper.get_embeddings_model()['doc']],
                             disabled=True,
                             key="embeddings_model_url")
        ...
        with st.expander("View documents in the knowledge base", expanded=False):
            ...
            WARN_MSG = 'No embeddings found. Copy paste your data in the text input' + \
                        ' and click on "Compute Embeddings" or drag-and-drop documents.'
            try:
