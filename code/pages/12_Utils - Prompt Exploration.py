'''Prompt Exploration page for Streamlit app.'''
...
def get_prompt():
    'Get prompt from the input'
    return f"{st.session_state['doc_text']}\n{st.session_state['input_prompt']}"

def customcompletion():
    'Create custom completion from the input'
...
def process_all(data):
    ...
    for doc in data_to_process.to_dict('records'):
        ...
        llm_helper.vector_store.add_prompt_result(doc['key'],
                                                  response.encode().decode(),
                                                  doc['filename'],
                                                  st.session_state['input_prompt'])
    st.session_state['data_processed'] = llm_helper.vector_store.get_prompt_results().to_csv(
                                            index=False)
...
if os.getenv('AAD_General_SG') in st.session_state['user_groups'] or \
    ...
    try:
        ...
        if len(data) == 0:
            ...
            with cols[1]:
                st.multiselect('Select documents',
                               sorted(set(data.filename.tolist())),
                               key='selected_docs')
            with cols[2]:
                st.text('-')
            ...
            with cols[3]:
                ...
                if st.session_state['data_processed'] is not None:
                    DOWNLOAD_DATA = st.session_state['data_processed']
                st.download_button(label='Download results',
                                   data=DOWNLOAD_DATA,
                                   file_name='results.csv',
                                   mime='text/csv',
                                   disabled=st.session_state['data_processed'] is None)
