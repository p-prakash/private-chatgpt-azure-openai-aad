...
class AzureTranslatorClient:
    ...
    def __init__(self, translate_key=None, translate_region=None, translate_endpoint=None):
        ...
        if os.getenv('VNET_DEPLOYMENT', 'false') == 'false':
            self.detect_endpoint = urljoin(self.translate_endpoint,
                                           f"/detect?api-version={self.api_version}")
            self.translate_endpoint = urljoin(self.translate_endpoint,
    ...
    def translate(self, text, language='en'):
        ...
        request = requests.post(self.detect_endpoint,
                                params=params,
                                headers=headers,
                                json=body,
                                timeout=120)
        ...
        if response[0]['language'] != language:
            ...
            request = requests.post(self.translate_endpoint,
                                    params=params,
                                    headers=headers,
                                    json=body,
                                    timeout=120)
    ...
    def get_available_languages(self):
        ...
        if os.getenv('VNET_DEPLOYMENT', 'false') == 'true':
            ...
            trans_res = requests.get(f"https://api.cognitive.microsofttranslator.com/languages?api-version={self.api_version}&scope=translation",
                             timeout=120)
            available_languages = trans_res.json()['translation']
        languages = {}
        for k, val  in available_languages.items():
            languages[val['name']] =  k
        return languages
